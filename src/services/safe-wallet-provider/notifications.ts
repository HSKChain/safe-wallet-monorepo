...
> = {
  SIGNATURE_REQUEST: (appInfo: AppInfo) => ({
    ...
    options: {
      body: `${appInfo.name} wants you to sign a message. Open HSKChain Safe to continue.`,
    },
  }),
  TRANSACTION_REQUEST: (appInfo: AppInfo) => ({
    title: 'Transaction request',
    options: {
      body: `${appInfo.name} wants to submit a transaction. Open HSKChain Safe to continue.`,
    },
  }),
}
